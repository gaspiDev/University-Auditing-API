...
class ExpenseRouter:
    ...
    async def create(
        expense_category: CategoryEnum,
        expense_for_creation: ExpenseForCreation,
        user: Annotated[dict, Depends(current_user)],
        session: AsyncSession = Depends(get_db),
    ):
        service = ExpenseService(session)
        expense_created_id = await service.create(
            expense_category, expense_for_creation, user["id"]
        )
        return {
            "status": 201,
            "message": f"Expense ID: {expense_created_id} successfully created.",
        }
    ...
    async def update(
        ...
        service = ExpenseService(session)
        expense_updated_id = await service.update(
            expense_for_update, category, user["id"]
        )

        return {
            "status": 200,
            "message": f"Expense ID: {expense_updated_id} is updated",
        }
    ...
    async def update(
        ...
        return {
            "status": 200,
            "message": f"Expense ID: {expense_deleted} successfully deleted",
        }
