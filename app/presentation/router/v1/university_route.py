...
class UniversityRoute:
    ...
    async def create(
        ...
        university_for_creation: UniversityForCreation,
        ...
        return {
            "status": 201,
            "message": f"University ID: {university_id} successfully created.",
        }
    ...
    async def update(
        university_for_update: UniversityForUpdate,
        user: Annotated[dict, Depends(current_user)],
        session: AsyncSession = Depends(get_db),
    ):
        service = UniversityService(session)
        ...
        return {
            "status": 200,
            "message": f"University ID: {updated_university_id} is updated.",
        }
    ...
    async def delete(
        ...
        return {
            "status": 200,
            "message": f"University ID: {university_deleted_id} successfully deleted.",
        }

    @router.get(
        "/{university_id}/total-expense/{year}", status_code=200, response_model=dict
    )
    async def total_expense_by_year(
        university_id: int, year: int, session: AsyncSession = Depends(get_db)
    ):
        service = UniversityService(session=session)
