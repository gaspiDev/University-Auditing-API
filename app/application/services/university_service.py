...
from app.application.schemas.university.university_for_creation import (
    UniversityForCreation,
)
...
class UniversityService:
    ...
    async def create(
        self, university_input: UniversityForCreation, user_id: int
    ) -> int:
        user = await self.user_service.read_by_id(user_id)
        if user.university:
            raise HTTPException(
                status_code=400, detail="Deans can be linked with only one university"
            )
        if university_input.budget_type.lower() == "public":
            ...
            raise HTTPException(
                status_code=400, detail="Invalid budget type. Must be public or private"
            )
    ...
    async def read_by_id(self, university_id: int) -> UniversityForView:
        ...
        if not university:
            raise HTTPException(
                status_code=404, detail=f"University ID: {university_id} doesn't exist."
            )
        university_for_view = UniversityForView(
            id=university.id,
    ...
    async def update(
        self, university_for_update: UniversityForUpdate, user_id: int
    ) -> int:
        user = await self.user_service.read_by_id(user_id)
    ...
    async def delete(self, user_id: int) -> int:
        ...
        if not user.university:
            raise HTTPException(
                status_code=400,
                detail=f"User ID: {user_id} must be linked with a University in order to delete it.",
            )
        university = await self.repository.read_by_id(user.university.id)
    ...
    async def total_expenses_by_year(self, university_id: int, year: int) -> float:
        expenses_by_year = await self.expense_service.total_expenses_by_id_and_year(
            university_id, year
        )
        return expenses_by_year

    async def under_budget(self, university_id) -> bool:
        total_expense = await self.expense_service.total_expenses_by_id_and_year(
            university_id
        )
        university = await self.repository.read_by_id(university_id)
