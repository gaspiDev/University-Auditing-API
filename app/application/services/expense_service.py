...
class ExpenseService:
    ...
    async def create(
        self, expense_category, expense_for_creation: ExpenseForCreation, dean_id: int
    ) -> int:
        dean = await self.user_service.read_by_id(dean_id)
        if not dean.university:
            raise HTTPException(
                status_code=404, detail="Must be linked with a University."
            )
        expense = Expense(
    ...
    async def read_by_id(self, expense_id: int) -> ExpenseForView:
        ...
        if not expense:
            raise HTTPException(
                status_code=404, detail=f"Expense ID: {expense_id} doesn't exist."
            )
        expense_for_view = ExpenseForView(
            id=expense.id,
            date=expense.date,
    ...
    async def update(
        self,
        expense_for_update: ExpenseForUpdate,
        ...
        if not expense:
            raise HTTPException(
                status_code=404,
                detail=f"Expense ID: {expense_for_update.id} doesn't exist",
            )
        if category:
    ...
    async def delete(self, expense_id: int, dean_id: int) -> int:
        ...
        if expense.university.dean_id != dean_id:
            raise HTTPException(
                status_code=403,
                detail="Forbiden, can't delete expenses of another university.",
            )
        expense_deleted = await self.repository.delete(expense.id)
        return expense_deleted.id

    async def total_expenses_by_id_and_year(
        self, university_id: int, year: int = None
    ) -> float:
        if year is None:
            year = datetime.now().year
        expenses = await self.repository.total_expenses_by_id_and_year(
            university_id, year
        )
        return expenses
