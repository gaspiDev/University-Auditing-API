...
class UserService:
    ...
    async def auth(self, dni, password) -> dict:
        ...
        if not user:
            raise HTTPException(
                status_code=401, detail="Unauthorized, dni or password are incorrect."
            )
        ...
        token = jwt.encode(
            encode, os.getenv("SECRET_KEY"), algorithm=os.getenv("HASH_ALGORITHM")
        )
    ...
    async def read_by_id(self, user_id: int) -> UserForView:
        ...
        if not user:
            raise HTTPException(
                status_code=404, detail=f"User ID: {user_id} doesn't exists."
            )
