...
class BudgetServices:
    ...
    async def create(self, budget_for_creation: BudgetForCreation) -> int:
        ...
        for b in budgets:
            if (
                (b.type == budget_for_creation.type)
                and (b.year == budget_for_creation.year)
                and (b.approved_by == budget_for_creation.approved_by)
            ):
                raise HTTPException(
                    status_code=400,
                    detail="A party can approve only one budget per year and type",
                )
    ...
    async def read_by_id(self, budget_id: int) -> BudgetForView:
        ...
        if not budget:
            raise HTTPException(
                status_code=404, detail=f"Budget ID {budget_id} doesn't exists."
            )
        budget_for_view = BudgetForView(
            id=budget.id,
            type=budget.type,
    ...
    async def update(self, budget_for_update: BudgetForUpdate) -> int:
        ...
        if not budget:
            raise HTTPException(
                status_code=404,
                detail=f"Budget ID: {budget_for_update.id} doesn't exist",
            )
    ...
    async def delete(self, budget_id: int) -> int:
        ...
        if not budget:
            raise HTTPException(
                status_code=404, detail=f"Budget ID: {budget_id} doesn't exist."
            )
